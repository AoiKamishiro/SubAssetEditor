...
namespace Coffee.Editors
{
    internal class SubAssetEditor : EditorWindow
    {
        ...
        private readonly List<Object> _referencingAssets = new();
        ...
        {
            GetWindow<SubAssetEditor>("Sub Asset");
        }
        ...
        private void OnSelectionChanged()
        {
            ...
            if (!_isLocked && active && _current != active && active is not SceneAsset && AssetDatabase.IsMainAsset(active))
            {
        ...
        private void OnSelectionChanged(Object active)
        {
            ...
            foreach (Object o in AssetDatabase.LoadAllAssetsAtPath(AssetDatabase.GetAssetPath(active)))
            {
                if (o == null)
                {
                    continue;
                ...
                SerializedProperty sp = new SerializedObject(o).GetIterator();
                sp.Next(true);
                ...
                while (sp.Next(true))
                {
                    ...
                    Object asset = sp.objectReferenceValue;
        ...
        private void AddSubAsset(IEnumerable<Object> assets)
        {
            // accepted object
            assets = assets
                .Where(x => x != _current && x is not SceneAsset && AssetDatabase.Contains(x) && !_subAssets.Contains(x));

            // Replace targets
            SerializedObject[] replaceTargets = AssetDatabase.LoadAllAssetsAtPath(AssetDatabase.GetAssetPath(_current))
            ...
            {
                Object newInstance = Instantiate(asset);
        ...
        private void OnGUI()
        {
            ...
            using (EditorGUILayout.ScrollViewScope scrollView = new(_scrollPosition))
            {
                ...
                using (new EditorGUILayout.HorizontalScope())
                {
                    ...
                    if (GUILayout.Button("Show Part"))
                    {
                        ShowPart();
                    }

                    if (GUILayout.Button("Delete Unused All"))
                    {
                        DeleteUnusedAll();
                    }
                }
                ...
                EditorGUI.indentLevel++;
                foreach (Object asset in _subAssets)
                {
                    Rect r = EditorGUILayout.GetControlRect(true);
                ...
                foreach (Object asset in _referencingAssets.Except(_subAssets))
                {
        ...
        {
            GUILayout.Space(5);
            Rect dropArea = GUILayoutUtility.GetRect(0, 20, GUILayout.ExpandWidth(true), GUILayout.ExpandHeight(false));
        ...
        {
            string exportDir = Path.GetDirectoryName(AssetDatabase.GetAssetPath(_current));
            string exportName = obj.name + " (Exported)." + GetFileExtension(obj);
            string uniquePath = AssetDatabase.GenerateUniqueAssetPath(Path.Combine(exportDir, exportName));
            AssetDatabase.CreateAsset(Instantiate(obj), uniquePath);
            AssetDatabase.SaveAssets();
            AssetDatabase.Refresh();
        }
        ...
        private void DeleteUnusedAll()
        {
            ...
            AssetDatabase.SaveAssets();
            AssetDatabase.Refresh();
            OnSelectionChanged(Selection.activeObject);
        }
        private void ShowAll()
        {
            Object active = Selection.activeObject;
            string assetPath = AssetDatabase.GetAssetPath(active);
            foreach (Object asset in AssetDatabase.LoadAllAssetsAtPath(assetPath).Where(x => x != null && !(x is GameObject || x is Component) && x != _current).Distinct())
            {
        ...
        private void ShowPart()
        {
            foreach (Object asset in _subAssets)
            {
                if (asset.GetType() == typeof(AnimatorStateTransition) || asset.GetType() == typeof(BlendTree) || asset.GetType() == typeof(AvatarMask) || asset.GetType() == typeof(AnimatorStateMachine) || asset.GetType() == typeof(AnimatorState) || asset.GetType().IsSubclassOf(typeof(StateMachineBehaviour)))
                {
                    ...
                    while (sp.Next(true))
                    {
                        if (sp.propertyPath == "m_ObjectHideFlags")
                        {
                            sp.intValue = 1;
                        }
                    }
